Meteor.publish("patterns", function (selector, options) {
    return Patterns.find({}, selector, options);
 });

Meteor.publish('singlePattern', function(id) {
	return id && Patterns.find(id);
});
...
Meteor.publish("subpatterns", function(postId) {
    return SubPatterns.find({subpatternA: postId});
});

Meteor.publish("profilePatterns", function (creator) {
    return Patterns.find({creator: creator});
});
